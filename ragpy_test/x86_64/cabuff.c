...
#define nrn_init _nrn_init__cabuff
#define _nrn_initial _nrn_initial__cabuff
#define nrn_cur _nrn_cur__cabuff
#define _nrn_current _nrn_current__cabuff
#define nrn_jacob _nrn_jacob__cabuff
#define nrn_state _nrn_state__cabuff
#define _net_receive _net_receive__cabuff 
#define states states__cabuff 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_cabuff", _hoc_setdata,
 0, 0
};
 /* declare global and static user variables */
#define Pb Pb_cabuff
 double Pb = 0;
#define taupump taupump_cabuff
 double taupump = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "area");
 	nrn_writes_conc(_mechtype, 0);
 	hoc_register_cvode(_mechtype, _ode_count, _ode_map, _ode_spec, _ode_matsol);
 	hoc_register_tolerance(_mechtype, _hoc_state_tol, &_atollist);
 	hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 cabuff /Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/cabuff.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double FARADAY = 96480;
 static double PI = 3.142857;
static int _reset;
static char *modelname = "Ca Buffers from PN Model in MATLAB";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcai = 1000.0 * ( ( ( ( - ica / 1000.0 ) * area * 1e-8 ) * ( 1.0 - ( Btot / ( cai + Kbuff + Btot ) ) ) / ( 2.0 * vshell * FARADAY ) ) + ( ( cai0 - cai ) / ( 17.7 * exp ( v / 35.0 ) ) ) ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dcai = Dcai  / (1. - dt*( (( 1000.0 * ( ( ( ( - ica / 1000.0 ) * area * 1e-8 ) * ( 1.0 - ( Btot / ( ( cai  + .001) + Kbuff + Btot ) ) ) / ( 2.0 * vshell * FARADAY ) ) + ( ( cai0 - ( cai  + .001) ) / ( 17.7 * exp ( v / 35.0 ) ) ) ) ) - ( 1000.0 * ( ( ( ( - ica / 1000.0 ) * area * 1e-8 ) * ( 1.0 - ( Btot / ( cai + Kbuff + Btot ) ) ) / ( 2.0 * vshell * FARADAY ) ) + ( ( cai0 - cai ) / ( 17.7 * exp ( v / 35.0 ) ) ) )  )) / .001 )) ;
  return 0;
}
 /*END CVODE*/
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/cabuff.mod";
static const char* nmodl_file_text = 
