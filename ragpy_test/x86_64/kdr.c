...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 KDR /Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/kdr.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "KDR channel from PN MATLAB/PYTHON model";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   DmDr = 1000.0 * ( miDR - mDr ) / tmDR ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 rate ( _threadargscomma_ v ) ;
 DmDr = DmDr  / (1. - dt*( ( ( 1000.0 )*( ( ( - 1.0 ) ) ) ) / tmDR )) ;
  return 0;
}
 /*END CVODE*/
...
static int  rate ( _threadargsprotocomma_ double _lv ) {
   double _lnumdr ;
 _lnumdr = 0.01 * ( ( _lv + 45.0 ) / ( 1.0 - ( exp ( - ( _lv + 45.0 ) / 5.0 ) ) ) ) ;
   tmDR = 1.0 / ( ( 0.01 * ( ( _lv + 45.0 ) / ( 1.0 - ( exp ( - ( _lv + 45.0 ) / 5.0 ) ) ) ) ) + ( 0.17 * exp ( - ( _lv + 50.0 ) / 40.0 ) ) ) ;
   miDR = _lnumdr * tmDR ;
    return 0; }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(mDr) - _p;  _dlist1[0] = &(DmDr) - _p;
 _slist2[0] = &(mDr) - _p;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/kdr.mod";
static const char* nmodl_file_text = 
