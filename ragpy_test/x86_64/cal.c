...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 CaL /Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/cal.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "CaL channel from PN MATLAB model i.e., without F_L,mod";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   DmcaL = 1000.0 * ( micaL - mcaL ) / tmcaL ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 rate ( _threadargscomma_ v , cai ) ;
 DmcaL = DmcaL  / (1. - dt*( ( ( 1000.0 )*( ( ( - 1.0 ) ) ) ) / tmcaL )) ;
  return 0;
}
 /*END CVODE*/
...
static int  rate ( _threadargsprotocomma_ double _lv , double _lcai ) {
   double _lnumcal ;
 _lnumcal = 1.6 / ( exp ( - 0.072 * ( _lv - 5.0 ) ) ) ;
   tmcaL = 1.0 / ( 1.6 / ( exp ( - 0.072 * ( _lv - 5.0 ) ) ) + ( 0.02 * ( _lv - 1.31 ) ) / ( exp ( ( _lv - 1.31 ) / 5.36 ) - 1.0 ) ) ;
   micaL = _lnumcal * tmcaL ;
    return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   gcaL = gcaLbar * ( pow( mcaL , 2.0 ) ) ;
   ica = 1000.0 * gcaL * ( v - ( 13.27 * log ( 4.0 / cai ) ) ) ;
   }
 _current += ica;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/jessicafeldman/Desktop/ragp/ragp/ragpy_test/mod/cal.mod";
static const char* nmodl_file_text = 
