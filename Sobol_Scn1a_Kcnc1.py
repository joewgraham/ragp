...
cond2 = soma(0.5).ch_Kcnc1_md74298.gk
...
for x, y in zip(a1, b1):
    ...
    soma(0.5).ch_Kcnc1_md74298.gk = (10**((y-50)/50))*cond2
    ...
    Kc_C = ("%.2E" % soma(0.5).ch_Kcnc1_md74298.gk)
  
#########################################    
    
    mylist2 = [1,2]  #Current strength
    plt.subplot(a, b, c)
    plt.rcParams.update({'font.size': 8}) 
    plt.title('C= {}'.format(c)+ ',  C(Na)= %s' %Na_C + '  C(Kb)= %s' %Kc_C)
    plt.ylim((-70,70))
    plt.xlabel("Time (ms)")
    plt.ylabel("V (mV)")    
    plt.grid(True)
    ...
    for iclamp.amp in mylist2: 
        ...
        for x1, y1 in zip(t, v):   
            if ((y1> 5) and (x1 >150 )): # If action potential     
                aparray1.append((x,y))
                aparray2.append((soma(0.5).ch_Scn1a_md264834.gNav11bar,soma(0.5).ch_Kcnc1_md74298.gk))
                AP = True
                break
        ...
        for x1, y1 in zip(t, v):  
            if ((x1< 50) and (y1>0)): #If spontaneous action potential 
                sparray1.append((x,y))
                sparray2.append((soma(0.5).ch_Scn1a_md264834.gNav11bar,soma(0.5).ch_Kcnc1_md74298.gk))
                SP = True
                break
            else:
                continue
